...
description: Dùng khi cần vẽ activity diagram có swimlane thật (mỗi vai trò 1 lane thẳng cột) cho quy trình đa vai trò, bằng PlantUML. Là lựa chọn mặc định khi quy trình có nhiều tương tác chéo lane.
...
* **Render qua plantuml.com** (`render.sh`) — máy này KHÔNG có Java. **TRADE-OFF**: nội dung diagram (tên lane/bước) gửi qua internet mỗi lần render. Nội dung nhạy cảm → không dùng skill này (xem Pitfalls).
* **AI viết source .puml, KHÔNG tự tính toạ độ** — PlantUML layout engine lo. Vai trò AI = mô tả đúng nghiệp vụ (lane/bước/nhánh/loop).
* **Compile phải PASS** trước khi báo xong. `render.sh` chạy **3 lớp**: (1) `puml-activity-lint.mjs` kiểm **cấu trúc luồng** TRƯỚC khi gửi server — `if` thiếu `endif`, thiếu `start`/`stop`, nhánh quyết định không nhãn, chỉ 1 lane; (2) HTTP; (3) `grep "Syntax Error"` nhúng trong SVG. Lint fail → **DỪNG, không render** (ảnh sai không bao giờ được tạo). Fail → sửa source, chạy lại (tối đa 2 lần).
* **Lint KHÔNG kiểm được "bước này có đúng lane không"** — gán sai vai trò (vd "Gọi cổng thanh toán" nằm lane Khách) là lỗi nghiệp vụ, máy chịu. Phải tự soi trên ảnh PNG + đối chiếu use case, và nói rõ trong report là phần này do người kiểm.
...
10. **Layout sạch, không rối** — node đặt logic, hạn chế mũi tên cắt chéo (xem Pitfalls — mũi tên).
...
9. Coverage-verify: mỗi decision fact-list thành 1 if/else? mỗi lane thành 1 |Lane|? mọi nhánh tới stop?
   ...
   ▼
10. Update srs/{feature}-flows.md updated + env note → changelog.md. Báo user mở .svg.
```
