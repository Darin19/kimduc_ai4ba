...
9) **Activity log** — set env `CLAUDE_SKILL_NAME=/sequence` + `CLAUDE_CHANGELOG_NOTE` (note: `added {flow-title} sequence`) TRƯỚC khi Write — hook append vào `docs/_shared/changelog.md` (không phụ thuộc spec.md tồn tại hay chưa, không còn routing/fallback). Update flows.md `updated: {date}`.
9.5. **Render-verify (BẮT BUỘC, chạy ngay sau Write)** — `node .claude/scripts/mermaid-verify.mjs --file docs/{feature}/srs/{feature}-flows.md`. Mermaid không render trong chat (đây là lý do skip L3), nên đây là cách duy nhất bắt lỗi cú pháp TRƯỚC khi báo "xong" thay vì để user tự phát hiện khi mở IDE.
   * Script chạy **3 tầng, báo tách riêng**: cú pháp compile · nhãn an toàn renderer · **ngữ nghĩa**.
     Tầng ngữ nghĩa cho sequence bắt: **lifeline chưa khai báo mà đã gửi/nhận message** (**error** —
     mermaid âm thầm tự tạo lifeline mới, hình lệch mà vẫn compile PASS; thường do gõ sai alias);
     khối `alt` không có `else` (**cảnh báo** — 1 nhánh điều kiện thì nên dùng `opt`).
...
`docs/{feature}/srs/{feature}-flows.md` — **append 1 section** `## Flow: {title}` với mermaid `sequenceDiagram` inline. File dùng CHUNG với `/activity` (mỗi flow 1 section, không tách file). Slim frontmatter (`type: srs-flows` / `feature` / `updated`).
