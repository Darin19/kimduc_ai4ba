...
```mermaid
stateDiagram-v2
    [*] --> unread : hệ thống tạo thông báo mới
    unread --> read : user chạm vào thông báo
    read --> archived : hệ thống dọn thông báo cũ
    archived --> [*]
```
...
```mermaid
stateDiagram-v2
    [*] --> active : mặc định khi tạo kênh
    active --> muted : user bấm Mute kênh
    muted --> active : user bấm Bật lại HOẶC tự hết hạn sau 30 ngày

    note right of muted
        Mute tối đa 30 ngày, tự bật lại
        (BR-smart-notification-001).
    end note
```
