---
name: userguide
description: Dùng khi cần viết hướng dẫn sử dụng/cẩm nang vận hành cho admin/CSKH từ các tài liệu BA đã có. `/userguide` hoặc `/userguide <feature>`.
...
* **Thông tin cấp dự án đọc + ghi vào profile** — thuật ngữ gọi người dùng cuối + domain sản phẩm quyết cách xưng hô và danh sách audience đề xuất ở Pha C.4: đọc `docs/_shared/project-profile.md` TRƯỚC khi hỏi audience (đừng liệt kê nhóm audience của domain khác); thiếu thì hỏi rồi đề xuất ghi vào profile. Per @../../rules/project-profile.md.
...
* **Update mode (file đã tồn tại)** → Read `{ug-index}` + trang cũ trước, L2 diff per file, set env note (hook ghi changelog.md). Outline cũ còn → đối chiếu trang mới/bỏ.
* **@author resolution** (`ba-conventions` Mục 1) cho changelog.md — KHÔNG ghi `owner` vào frontmatter (đã diet).
...
[Pha D.5] Ảnh minh họa — HỎI user chọn 1 trong 3: (A) auto-chụp Playwright (cần nguồn+quyền)
        ...
        │
        ▼
[Pha E] L1 plan preview (prose) → Write per-section (nếu chưa) → update {ug-index} (env note → changelog.md).
        │
        ▼
...
12.8. **(A) Check Playwright**: `node .claude/skills/userguide/engine/check-playwright.mjs` (đọc exit code + JSON). Thiếu module (exit 2) hoặc browser (exit 3) → **hỏi user "cài {module|chromium} không?"** rồi mới chạy lệnh trong `hint` (`npm install` / `npx playwright install chromium`). **KHÔNG tự cài im lặng.**
...
12.10. **(A) Chạy** `node .claude/skills/userguide/engine/capture.mjs <job.json>` **từ repo root**. Engine: chờ font ready + freeze animation → định vị element qua locator (iframe-aware) → inject SVG overlay TRƯỚC screenshot → ghi PNG + `.png.sha1`. Ảnh không đổi (sha1 khớp) tự skip; ép chụp lại đặt env `UG_FORCE=1`.
...
17) **Gom OQ:** grep mọi `<!-- TBD ... OQ-{n} -->` trong các trang vừa viết → in danh sách + hỏi `Resolve giờ? (Y / skip / ids)`.
    ...
    * Mỗi trang bị sửa: set `CLAUDE_CHANGELOG_NOTE` trước Edit (hook ghi changelog.md).
18) **Final report:**
    ```
...
Mỗi lần chạy sinh **1 file `.html` lộ ra** + **1 folder bundle cùng tên**:

| Path | Nội dung |
|---|---|
| `docs/userguide/{feature}-userguide.html` | **Cửa vào DUY NHẤT** — docs-style light-only, self-contained |
| `docs/userguide/{feature}-userguide/index.md` | Master metadata + bảng Sections |
| `docs/userguide/{feature}-userguide/data.js` | Nội dung nhúng cho file `.html` |
| `docs/userguide/{feature}-userguide/pages/{slug}.md` | Trang cẩm nang — **zero frontmatter** |
| `docs/userguide/{feature}-userguide/images/{slug}.png` | Ảnh chụp thật (Playwright) hoặc placeholder |

Toàn sản phẩm → `userguide.html` + `userguide/`. Top-level `docs/userguide/` chỉ thấy file `.html`.
