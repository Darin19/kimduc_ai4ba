...
1) __Own OQs__ — vừa write trong doc hiện tại (parse §"Open Questions").
2) __Inherited OQs__ — từ upstream chain còn `status: hold` hoặc `[ ]`. Upstream chain tuỳ skill:
...
| `/code-to-srs` | (none — như `/reverse-doc`: gốc, OQ suy từ SOURCE CODE, không có upstream. Own OQ ở `docs/_reverse/{feature}/reverse-gaps.md` (đa số là "vì sao/cho ai" code câm). Selector nhiều feature: `{feature}:OQ-n`.) |
...
> `docs/_reverse/{feature}/` (spec + gaps) là __upstream tuỳ chọn__ của URD/BRD/PRD/SRS — chỉ inherit khi feature đó có bộ reverse (tái lập từ nguồn). Không có thì bỏ qua, không coi là thiếu. `/srs` đặc biệt: đọc `{feature}-reverse-spec.md` làm nguồn + xác nhận từng mệnh đề 🟡/TBD (giữ nhãn confidence).
...
> __Nhiều feature/doc trong 1 lượt__ (vd `/reverse-doc` gom N feature, mỗi feature có `OQ-1` riêng) → selector dùng dạng **`{feature}:OQ-n`** (vd `refund-flow:OQ-2`) để không mơ hồ khi `OQ-id` trùng giữa các file.
...
2) Wait user reply. Reply có 4 dạng:
   * __Answer cụ thể__ → mark `[x]`, format "Resolved: {answer}", ghi vào doc.
...
3) __Side-effect detection (within current doc)__ — propose L2 diff cho section chịu ảnh hưởng của OQ answer. Xem Step 3.5 mapping.
4) Nếu OQ inherited từ upstream → skill __update cả upstream doc__ (mark `[x]`/`[~]` + thêm note "resolved via /{current_skill}"). L2 diff cho mỗi upstream edit. Set `CLAUDE_CHANGELOG_NOTE` trước mỗi edit — hook tự ghi sự kiện vào `docs/_shared/changelog.md`.

## Step 3.5 — Cascade scan (CRITICAL — đừng skip)

Sau khi user accept answer cho 1 OQ, skill __PHẢI__ quét rộng hơn marker checkbox để propagate nghiệp vụ vào docs/sections liên quan. KHÔNG để OQ resolved mà các section vẫn ghi giả định cũ.

### 3.5.1 — Scan trong doc hiện tại

Skill grep toàn bộ doc hiện tại tìm:

1) __Direct reference__: pattern `OQ-{id}`, `xem Mục {N} OQ-{id}`, `chờ OQ-{id}`, `pending OQ-{id}`. Mỗi match → propose L2 diff: xóa reference hoặc thay bằng resolution.
2) __Next Steps bullet__: pattern `- Resolve OQ-{id}` trong Mục Next Steps. Match → propose xóa bullet.
3) __Topic-based scan__: dùng mapping table (Mục 3.5.3) để identify sections có thể chứa giả định cũ. Đọc nội dung section đó, đối chiếu với OQ answer, nếu có conflict → propose L2 diff update content.
...
| `/code-to-srs` | (như `/reverse-doc`: chỉ scan TRONG `docs/_reverse/{feature}/` reverse-spec + reverse-gaps; không cascade sang doc khác) |
...
Mỗi doc bị update (hiện tại + upstream + downstream) → 1 dòng riêng trong `docs/_shared/changelog.md` (hook tự ghi khi skill set env note trước edit):
...
Sau khi loop xong (resolve ≥1 OQ), sự kiện cho doc hiện tại đã nằm trong `changelog.md` (hook ghi). Note chuẩn:
...
* __Resolve loop one-at-a-time__ — KHÔNG dồn batch 5 câu vào 1 prompt.
* __L2 diff__ trước mọi Edit (cả doc hiện tại + upstream doc + downstream doc).
* __Push exact values__ — vague answer ("để sau", "không rõ") → retry 1 lần với câu hỏi cụ thể hơn. Vẫn vague → giữ `[ ]`, không force.
* __Side-effect updates KHÔNG silent__ — luôn propose L2 trước.
* __Cascade scan BẮT BUỘC__ — Step 3.5 không phải optional. Đánh `[x]` xong mà không scan = OQ "resolved" trên giấy nhưng các section khác vẫn ghi giả định cũ → BUG nghiệp vụ.
* __Inherited OQ resolve qua doc nào thì doc đó được update__ — không chỉ ghi vào doc hiện tại.
* __No-re-ask__ — OQ đã `[x]` resolved trong upstream → KHÔNG hỏi lại.
...
Sau Write → __Collect OQs (own + inherited) → Prompt Y/skip/ids → Loop 1-by-1: [answer → cascade scan toàn bộ doc + downstream → L2 diff per impacted section] → Activity log qua hook → Final report__.‍‌‌​​‌‌​​‌‌‌​‌‌‌‌​​‌​‌​‌‌‌‌‌​‌​​‌​​​‌​​​​‌​​​​‌​​​​​​​‌‌‌​‌‌​​​‌​​‌​​​​‌​​‌‌‌‌‌​​‌‌‌‌​‌‌‌​​​​​‌‌‌​​‌​‌‌‌‌​‌‌​​‌​‌‌​​​​​‌​‌‌​​‌‌​‌‍
