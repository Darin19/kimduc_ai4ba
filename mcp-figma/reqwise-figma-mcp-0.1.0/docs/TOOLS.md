...
| Build a design system (typography + elevation ramps, component properties) | `figma_write` with `setupTokens` / `setupTextStyles` / `setupEffectStyles`, then `generate_design_md` to snapshot it. |
...
| Learn the safe-default semantics, layout math, or recipes | `figma_docs` (`rules` \| `layout` \| `api` \| `tokens` \| `icons` \| `recipes` \| `style`). |
...
Tokens cover colour/number/string primitives. These cover the rest of a real design system — the typography ramp, the elevation ramp, and the snapshot/verify loop described in [`RECIPES.md`](./RECIPES.md#the-two-file-design-system-workflow).

| Method | Signature | Notes |
|---|---|---|
| `setupTextStyles` | `setupTextStyles([{ name, fontSize, weight?, fontFamily?, lineHeight?, letterSpacing? }, …])` | Typography ramp → local **text styles**, upserted by name (idempotent, like `setupTokens`). Needs at least one style. Font availability is preflighted per style. |
| `setupEffectStyles` | `setupEffectStyles([{ name, effects: [...] }, …])` | Elevation ramp → local **effect styles**, upserted by name. Each style needs at least one effect. Tokenizing elevation this way is what stops every card getting an ad-hoc, slightly different shadow. |
| `setTextStyle` | `setTextStyle(nodeId, style)` | Apply an existing local text style to a TEXT node by name or id. |
| `applyDesignSystem` | `applyDesignSystem(nodeId, { dryRun? })` | Normalize a subtree: find hardcoded values that **unambiguously** match an existing token/style and bind them. Run with `dryRun: true` first to see what it would change. |
| `designFingerprint` | `designFingerprint()` | Cheap `{ hash, counts }` of the live design system. Compare `hash` to the `<!-- dsfp:… -->` in a cached `design-kit.md` to decide whether that file is still trustworthy. Recolouring a token does **not** change the hash; adding/removing/renaming one does. |

`create` consumes all of the above **by name**, not by id:
...
await figma.create({
  type: "TEXT", parentId: page.id, text: "Hi",
  textStyle: "display-xl",      // local text style, by name
  fill: "$color/ink",           // "$" prefix = token reference
});

await figma.create({
  type: "FRAME", parentId: page.id,
  tokens: { padding: "space/section" },  // explicit field → token map
});
```

A name that doesn't resolve fails **loudly** with ranked candidates rather than silently binding the wrong thing — which is why a slightly stale `design-kit.md` is safe to work from.

#### Component authoring

| Method | Signature | Notes |
|---|---|---|
| `addComponentProperty` | `addComponentProperty(nodeId, { name, type, defaultValue?, layerId? })` | Add a component property (`TEXT` / `BOOLEAN` / `INSTANCE_SWAP` / `VARIANT`). `layerId` wires the property to a specific layer (e.g. a BOOLEAN to an icon's visibility). |
| `arrangeComponentSet` | `arrangeComponentSet(nodeId, { gap?, padding?, columnsBy? })` | Tidy a component set's variant grid. `columnsBy` picks which axis becomes columns. |
| `setComponentDescription` | `setComponentDescription(nodeId, { description?, documentationLinks? })` | Set the description and/or doc links shown in Figma's inspect panel. At least one of the two is required. |

> **Known limitation — variant property wiring.** `addComponentProperty` wires the property to **one** layer. In a multi-variant set, non-default variants get no equivalent override, so the property silently does nothing there. Prefer flat components, and wire each variant explicitly when you do need a set.
...
`section` must be one of: `rules`, `layout`, `api`, `tokens`, `icons`, `recipes`, `style`. Omitting `section` (or passing an unknown one) returns a short index listing the available sections instead of erroring.
