...
export async function executeWrite(code: string, session: Session, deps: ExecutorDeps): Promise<WriteResult> {
  ...
  // SANDBOX HARDENING — reparent injected host objects onto CONTEXT prototypes.
  //
  // Not injecting host built-ins fixed `({}).constructor.constructor` (a plain
  // object literal is created in the context realm). But `figma`, `state` and
  // `console` are objects we build in the HOST realm and inject directly, so
  // their prototype chain still terminates at the HOST Object/Function. That
  // left a live escape: `figma.constructor.constructor("return process")()`
  // (and the same via `state`/`console`) reached the host Function constructor,
  // whose evaluated code runs in the host realm with the real `process` — a
  // native `process.binding('fs')` file-read / `spawn_sync` primitive.
  //
  // Fix: run a bootstrap INSIDE the context that captures the context realm's
  // Object/Function and re-points every injected object's prototype at the
  // context's Object.prototype. After this, `injected.constructor` is the
  // context Object, `injected.constructor.constructor` is the context Function
  // (banned/host-free), and no injected value bridges back to the host realm.
  // We freeze the rewired prototypes so user code can't restore them.
  ...
  // and the bootstrap reparenting (above), `this.constructor.constructor`,
  // `({}).constructor.constructor`, and `figma/state/console.constructor.
  // constructor` all resolve to the CONTEXT realm's Function — which has no
  // host process/require — instead of the host realm's.
...
function argsToParams(op: AnyOperation, args: unknown[]): Record<string, unknown> {
  ...
  switch (op) {
    ...
    case "add_component_property":
      ...
      // or a single object with nodeId inside.
      return isObj(first)
        ? (first as Record<string, unknown>)
    ...
    case "apply_design_system":
      // applyDesignSystem(nodeId, {dryRun}) → {nodeId, ...opts}.
      return isObj(first)
        ? (first as Record<string, unknown>)
        : { nodeId: first, ...(isObj(second) ? (second as object) : {}) };
    case "create":
