...
const needsNodeId: Record<string, true> = {
  ...
  zoom_to_fit: true,
  layout_audit: true,
  audit_design_system: true,
  export_node: true,
  set_gradient: true,
  set_effects: true,
  set_reactions: true,
  set_text_style: true,
  apply_design_system: true,
...
const OP_SCHEMAS: Partial<Record<Operation, z.ZodTypeAny>> = {
  ...
  audit_design_system: z.object({ nodeId }).passthrough(),
  ...
  // Normalize a subtree: bind unambiguous hardcoded values to their token/style.
  apply_design_system: z
    .object({ nodeId, dryRun: z.boolean().optional() })
    .passthrough(),
