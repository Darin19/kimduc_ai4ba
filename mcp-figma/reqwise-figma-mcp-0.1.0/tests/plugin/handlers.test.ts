...
describe("Plugin handlers - safe defaults", () => {
  ...
  describe("create - nested children", () => {
    it("children array builds a subtree in one call (index 0 first)", () => {
      // create({ type:"FRAME", children:[A, B] }) must create A then B as
      // children of the frame — not drop them, and not reparent to the page.
      const spec = {
        type: "FRAME",
        layoutMode: "VERTICAL",
        children: [
      ...
      // Recursion parents each child to the just-created frame; array order is
      // z-order (title below sub in the layout stack).
      expect(spec.children[0]!.name).toBe("title");
    });

    it("nested children recurse to arbitrary depth", () => {
      // A card containing a row containing two labels — one create call.
      const spec = {
        type: "FRAME",
        name: "card",
        children: [
      ...
      const row = spec.children[0]! as { children: unknown[] };
      expect(row.children).toHaveLength(2);
    });

    it("child parentId is forced to the parent, ignoring any parentId on the child spec", () => {
      // A child spec's own parentId must not win — the recursion sets it to the
      // freshly created parent so the subtree can never leak to the page.
      const childSpec = { type: "TEXT", parentId: "9:9", characters: "x" };
      const forcedParentId = "1:1";
      const effective = { ...childSpec, parentId: forcedParentId };

      expect(effective.parentId).toBe("1:1");
    });

    it("createVariants base spec children propagate to every variant", () => {
      // createVariants builds each variant via create({ ...baseSpec }); since
      // baseSpec carries children, every variant now inherits the same anatomy.
      const baseSpec = {
        type: "COMPONENT",
        children: [
      ...
      expect(baseSpec.children.map((c) => c.name)).toEqual(["icon", "label"]);
    });
  });
