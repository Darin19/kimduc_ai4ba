...
```
 ...
 └──────────────────────────────────────────────────────┘
        ...
        ▼
 ┌──────────────────────────────────────────────────────┐
 │ BƯỚC 6 — MÁY CHẤM NGỮ PHÁP (chặn trước khi vẽ)        │
 │  Skill tạo file nguồn .puml, rồi cho một chương trình │
 │  kiểm tra tự động soi từng dòng: nét nối actor có     │
 │  đúng kiểu không, <<include>>/<<extend>> có vẽ đúng   │
 │  chiều không, có khung ranh giới chưa (xem Mục 7).    │
 │  SAI → DỪNG, không render. Sửa xong mới đi tiếp.      │
 └──────────────────────────────────────────────────────┘
        │
        ▼
 ┌──────────────────────────────────────────────────────┐
 │ BƯỚC 7 — Render thành ảnh                              │
 │  Gửi nội dung sang plantuml.com để ra ảnh .svg.       │
 │  Ảnh lỗi thì tự sửa tối đa 2 lượt.                    │
 └──────────────────────────────────────────────────────┘
        │
        ▼
 ┌──────────────────────────────────────────────────────┐
 │ BƯỚC 8 — Tự soi nghiệp vụ + cập nhật index             │
 │  Máy chỉ chấm được ngữ pháp, KHÔNG chấm được đúng     │
 │  nghiệp vụ — nên skill tự soi lại: actor có trùng vai │
 │  không, quan hệ có khớp mô tả use case không.         │
 │  Ảnh cùng bảng Actors/Relationships được nhúng vào    │
 │  file use case index của feature.                     │
 └──────────────────────────────────────────────────────┘
        │
        ▼
...
## 7. Có ảnh đẹp KHÔNG có nghĩa là vẽ đúng — nên có máy chấm ngữ pháp

Đây là điều dễ gây hiểu nhầm nhất, và nó xuất phát từ một sự cố có thật.

Sơ đồ use case được viết bằng chữ (mã nguồn), rồi mới dựng thành ảnh. Vấn đề nằm ở chỗ: **một sơ đồ sai ngữ pháp UML vẫn dựng ra ảnh rất đẹp.** Vẽ nét nối giữa người dùng và chức năng bằng mũi tên (sai — phải là nét thẳng không mũi tên), vẽ `<<include>>` chỉa ngược chiều, hay quên khung ranh giới hệ thống — tất cả đều cho ra một tấm hình trông hoàn toàn bình thường. Nhìn ảnh không phát hiện được.

Đã từng xảy ra đúng như vậy: một sơ đồ mắc 6 lỗi UML cùng lúc mà vẫn render thành công, và vì "có ảnh rồi" nên được coi là đã xong.

Vì thế bây giờ có một **chương trình kiểm tra tự động chạy TRƯỚC khi render**. Nó soi từng dòng theo bộ luật cố định: nét nối actor–use case phải đúng kiểu, `<<include>>` và `<<extend>>` phải đúng chiều và đúng chỗ, phải có khung ranh giới. **Sai một lỗi là dừng hẳn, không cho render ra ảnh** — chứ không phải cảnh báo rồi vẫn chạy tiếp.

Lý do phải làm chặt tới mức đó: cách viết đúng ở đây **ngược với thói quen**. Ở hầu hết loại sơ đồ khác, nối hai thứ với nhau thì dùng mũi tên. Riêng use case diagram, nét nối giữa người dùng và chức năng **không được** có mũi tên. Một lời dặn suông rất dễ bị quên; một chương trình chặn thì không.

Nhưng cần nhớ ranh giới của nó: **máy chỉ chấm được ngữ pháp, không chấm được nghiệp vụ.** Một sơ đồ có thể qua hết bộ luật mà vẫn sai — ví dụ gán nhầm chức năng cho actor không có quyền đó, hoặc tách một actor thành hai vai trùng nhau. Phần đó vẫn phải người đọc soi. Vì vậy khi báo kết quả, skill tách rõ hai mức: *ngữ pháp đã được máy kiểm* và *nghiệp vụ thì bạn cần tự xác nhận*.

***

## 8. Kết quả để ở đâu, và vì sao không có file `.md` riêng cho diagram?
...
## 9. Điều `/usecase-diagram` KHÔNG hỏi bạn (và vì sao)
