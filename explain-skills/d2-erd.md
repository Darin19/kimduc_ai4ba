...
```
 ...
 └──────────────────────────────────────────────────────┘
        ...
        ▼
 ┌──────────────────────────────────────────────────────┐
 │ BƯỚC 4B — ĐỐI CHIẾU với bản /erd gốc (bắt buộc)       │
 │  Ra được ảnh CHƯA phải là xong. Nếu tính năng đã có   │
 │  bản vẽ bằng /erd, hệ thống so hai bên: có bảng nào   │
 │  thừa/thiếu, liên hệ nào lệch không.                  │
 │  Lệch → báo để sửa cho khớp (xem Mục 5).              │
 └──────────────────────────────────────────────────────┘
        │
        ▼
 ┌──────────────────────────────────────────────────────┐
...
## 5. Kiểm tra công cụ trước — và vì sao "ra được ảnh" vẫn CHƯA phải là xong
...
**Vẽ ra được ảnh (Bước 4).** Sau khi viết xong phần mô tả sơ đồ, hệ thống nhờ công cụ D2 "vẽ thật" ra file ảnh. Nếu phần mô tả có lỗi cú pháp khiến vẽ không ra (chỗ hay gặp nhất là quên bọc dấu ngoặc quanh những giá trị có ký tự đặc biệt, ví dụ danh sách lựa chọn "miễn phí | trả phí"), hệ thống **tự đọc lỗi, sửa lại, vẽ lại**.

**Nhưng ra được ảnh KHÔNG có nghĩa là vẽ đúng (Bước 4B).** Đây là điểm dễ nhầm nhất. Một sơ đồ thiếu hẳn một bảng, hoặc nối sai liên hệ giữa hai bảng, vẫn cho ra tấm hình đẹp như thường — công cụ vẽ chỉ biết cú pháp có hợp lệ không, nó không biết nghiệp vụ của bạn.

Rủi ro này lớn hơn bình thường ở đây, vì cùng một "bức tranh dữ liệu" có thể tồn tại **ba bản** song song: bản `/erd` (Mermaid), bản `/d2-erd` (lệnh này) và bản `/dbdiagram` (mức database). Sửa một bản mà quên hai bản kia là chuyện rất dễ xảy ra — và mỗi file tự nó vẫn hợp lệ, nên không công cụ đơn lẻ nào phát hiện được.‍‌‌​​‌‌​​‌‌‌​‌‌‌‌​​‌​‌​‌‌‌‌‌​‌​​‌​​​‌​​​​‌​​​​‌​​​​​​​‌‌‌​‌‌​​​‌​​‌​​​​‌​​‌‌‌‌‌​​‌‌‌‌​‌‌‌​​​​​‌‌‌​​‌​‌‌‌‌​‌‌​​‌​‌‌​​​​​‌​‌‌​​‌‌​‌‍

Vì vậy sau khi vẽ xong, hệ thống **đối chiếu bản vừa vẽ với bản `/erd` gốc** (bản `/erd` được coi là bản chuẩn): có bảng nào bên này có mà bên kia không, có liên hệ nào lệch nhau không. Phát hiện lệch thì báo để sửa cho khớp, thay vì để ba bản trôi mỗi bản một hướng. Nếu tính năng chưa từng vẽ bằng `/erd` thì bỏ qua bước đối chiếu này (không có gì để so).
